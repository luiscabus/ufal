...
int intersect_list(struct_node *head, int item, struct_node **interseccao) {
	struct_node * current = head;
	...
	while (current != NULL) {
		if (item == current->item) {
			...
			if (search_list(*interseccao, item)) {
				
			} else {
				if (*interseccao == NULL || (*interseccao)->item >= item) {
					inserir_no_inicio(&*interseccao, item);		
				} else {
					inserir_ordenado(&*interseccao, item);
				}
			}
...
int main() {
	...
	struct_node *interseccao = NULL; // Declara ponteiro do tipo nó, chamada head, que aponta para vazio
	...
	while (head2 != NULL) {
		intersect_list(head1, head2->item, &interseccao);
		head2 = head2->next;
	}
